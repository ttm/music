...
def stretches(x, durations=[1, 4, 8, 12], sample_rate=44100):
    ...
    Examples
    --------
    >>> asound = stereo_horizontal_stack(*[V(f=i, fv=j) for i, j in zip([220,440,330,440,330],
                                                  [.5,15,6,5,30])])
    >>> s = stretches(asound)
    >>> s = stretches(asound,durations=[.2,.3]*10+[.1,.2,.3,.4]*8+[.5,1.5,.5,1.,5.,.5,.25,.25,.5, 1., .5]*2)
    >>> W(durations, 'stretches.wav')
    Notes
    -----
    ...
    for ss in durations:
        if ns/ss >= 1:
            indexes = np.arange(0, l, ns / ss).round().astype(np.int64)
        else:
            indexes = np.arange(0, l - 1, ns / ss).round().astype(np.int64)
        if stereo:
            segment = x[:, indexes]
        else:
            segment = x[indexes]
        sound.append(segment)
    sound_ = stereo_horizontal_stack(*sound)
    return sound_
