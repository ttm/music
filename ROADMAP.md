...
## First: correctness patch, intended for 1.8.1
...
- Implementation completed in the working tree on 2026-09-16. Regression
...
- Keep changes under `Unreleased` until the release is cut. Publishing,
  tagging and updating the release citation are separate release steps.

## Next: measure whether the tests detect wrong answers

Run targeted mutation testing on normalization, export and session
envelopes: deliberately change an operation and check that an assertion
fails. Start with these modules and inspect surviving mutations before
expanding the run. Track the results, runtime and any accepted survivors
in [issue #113](https://github.com/ttm/music/issues/113).
...
- Reconcile release instructions with automation. The release gate omits
  examples, and the article, MASS and vocabulary checks described as
  release-time checks are not invoked by `tools/release.py`.
- Add an installed-wheel smoke check outside the checkout and consider
  macOS/Windows CI alongside the existing Ubuntu Python 3.10–3.14 matrix.
  The wheel check was run manually for this patch; automating it remains.
